...
mongoose.connect("mongodb+srv://amanaasti04:amanaasti04@cluster0.csv164k.mongodb.net/EComm")
...
app.post("/signup", async (req,res)=>{
    ...
    if(check){
        return res.status(400).json({success : false,err : "existing user found with same email address"})
    }
    let cart = {}
    for(let i =0;i<300;i++){
        cart[i]=0
    ...
    const token = jwt.sign(data,'salt_for_privacy')
...
const fetchUser = async(req,res,next)=>{
    ...
    if(!token) {
        ...
        try {
            const data = jwt.verify(token,"salt_for_privacy")
            req.user = data.user
            next()
        } catch (error) {
...
app.listen(port,(error)=>{
    if(!error){
        ...
        console.log(`error to running port`);
    }
})